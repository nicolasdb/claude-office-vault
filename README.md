...
3. **claude-office plugin** handles automation (see [plugin README](https://github.com/your-org/claude-office-plugin) for details):
   ...
   - **Session-end hook** logs activity to `team/<you>/activity/` (one file per repo) and commits your folder
...
team/
  ...
  <person>/                — Per-person folder
    ...
    activity/              — Per-repo session logs, auto-generated by session-end hook
      activity.md          — Default log (and activity-<repo>.md per project)
projects/
...
| `activity/` | Automatic per-repo session logs (`activity.md`, `activity-<repo>.md`) | Plugin (session-end hook) |
